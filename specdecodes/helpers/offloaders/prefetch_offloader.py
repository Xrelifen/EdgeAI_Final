...
class PrefetchOffloader:
    def __init__(self, model, device_map, record_stream=False):  
        ...
        def create_pre_hook(next_layer, device):
        ...
        for i in range(len(layer_order)):
            ...
            if i + 1 < len(layer_order):
                next_device = device_map.get(layer_order[i + 1])
                next_layer = find_child(model, layer_order[i + 1])
            else:
                next_device = None
                next_layer = None
            
            if next_layer is not None and next_device == "cpu":
                current_layer.register_forward_pre_hook(create_pre_hook(next_layer, device=gpu_device)) # cur_device
