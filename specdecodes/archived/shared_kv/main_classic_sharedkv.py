...
from ..models import SSM_SharedKV, LLM_Last_Layers
...
def train_one_epoch(model, llm_last, train_loader, optimizer, scheduler, train_config, epoch, num_epochs, accelerator, run=None):
    ...
    for idx, data in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch + 1}/{num_epochs}")):
        with accelerator.accumulate(model):
            with torch.no_grad():
                t_logits, t_hidden_states, (target_QK, target_V) = llm_last(data["target"], output_hidden_states=True, output_intermediates=True)
            s_hidden_states = model(input_ids=data["input_ids"], target_QK=target_QK, target_V=target_V, attention_mask=data["attention_mask"])[0]
            s_logits = llm_last(s_hidden_states, head_only=True)
            
            loss, ploss, vloss = calculate_loss(data["loss_mask"], s_hidden_states, t_hidden_states, s_logits, t_logits, train_config)
...
def main(args):
    ...
    llm_last = LLM_Last_Layers(llm, keep_layers_num=1)
    ...
    if args.pretrained is not None:
        ...
        model = SSM_SharedKV.from_pretrained(args.pretrained, config=draft_config)
    else:
        logger.info("Loading draft model...")
        model = SSM_SharedKV(config=draft_config)
