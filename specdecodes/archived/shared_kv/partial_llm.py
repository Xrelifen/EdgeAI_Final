from copy import deepcopy
import torch
import torch.nn as nn
from . import modeling_llama_training
from . import modeling_llama

class LLM_Last_Layers(nn.Module):
    def __init__(self, llm, keep_layers_num=1):
        super().__init__()
        begin_idx = llm.config.num_hidden_layers - keep_layers_num
        config = deepcopy(llm.config)
        config.org_num_hidden_layers = config.num_hidden_layers
        config.num_hidden_layers = keep_layers_num
        self.keep_layers_num = keep_layers_num
        self.config = config
        
        lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        lm_head.weight.data = llm.lm_head.weight.data
        self.lm_head = lm_head
        
        if keep_layers_num > 0:
            model = modeling_llama.LlamaModel(config)
            model.norm.weight.data = llm.model.norm.weight.data
            for idx, decoder_layer in enumerate(model.layers):
                real_idx = begin_idx + idx
                decoder_layer.self_attn.layer_idx = real_idx # assigning correct layer index for kv_cache
                for param, llm_param in zip(decoder_layer.parameters(), llm.model.layers[real_idx].parameters()):
                    param.data = llm_param.data
            del model.embed_tokens
            self.model = model

        self.freeze() # defaults set all requires_grad to False
        
    def freeze(self):
        for param in self.parameters():
            param.requires_grad = False
    
    def unfreeze(self):
        for param in self.parameters():
            param.requires_grad = True
            
    #dtype
    @property
    def dtype(self):
        return self.lm_head.weight.dtype
        
    def forward(self, hidden_states, head_only=False, output_hidden_states=False, *model_args, **kwargs):
        if head_only:
            return self.lm_head(hidden_states)
        
        if self.keep_layers_num > 0:
            outputs = self.model(inputs_embeds=hidden_states, *model_args, **kwargs)
            hidden_states = outputs.last_hidden_state

        logits = self.lm_head(hidden_states)
        
        output = (logits,)
        if output_hidden_states:
            output = output + (hidden_states,)
            
        # return output, if only logits is needed, decompose it
        return output if len(output) > 1 else output[0]

    
class LLM_Last_Layers_KV(nn.Module):
    def __init__(self, llm, keep_layers_num=1):
        super().__init__()
        begin_idx = llm.config.num_hidden_layers - keep_layers_num
        config = deepcopy(llm.config)
        config.org_num_hidden_layers = config.num_hidden_layers
        config.num_hidden_layers = keep_layers_num
        config._attn_implementation = "eager"
        self.keep_layers_num = keep_layers_num
        self.config = config
        
        lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        lm_head.weight.data = llm.lm_head.weight.data
        self.lm_head = lm_head
        
        if keep_layers_num > 0:
            model = modeling_llama_training.LlamaModel(config)
            model.norm.weight.data = llm.model.norm.weight.data
            for idx, decoder_layer in enumerate(model.layers):
                real_idx = begin_idx + idx
                decoder_layer.self_attn.layer_idx = real_idx # assigning correct layer index for kv_cache
                for param, llm_param in zip(decoder_layer.parameters(), llm.model.layers[real_idx].parameters()):
                    param.data = llm_param.data
            del model.embed_tokens
            self.model = model

        self.freeze() # defaults set all requires_grad to False
        
    def freeze(self):
        for param in self.parameters():
            param.requires_grad = False
    
    def unfreeze(self):
        for param in self.parameters():
            param.requires_grad = True
            
    #dtype
    @property
    def dtype(self):
        return self.lm_head.weight.dtype
         
    def forward(self, hidden_states, head_only=False, output_hidden_states=False, output_intermediates=False, *model_args, **kwargs):
        if head_only:
            return self.lm_head(hidden_states)
        
        QK_weight, V_weight = None, None
        if self.keep_layers_num > 0:
            if not output_intermediates:
                outputs = self.model(inputs_embeds=hidden_states, output_intermediates=output_intermediates, *model_args, **kwargs)
            else:
                outputs, (QK_weight, V_weight) = self.model(inputs_embeds=hidden_states, output_intermediates=output_intermediates, *model_args, **kwargs)
            hidden_states = outputs.last_hidden_state

        logits = self.lm_head(hidden_states)
        
        output = (logits,)
        if output_hidden_states:
            output = output + (hidden_states,)
        if output_intermediates:
            output = output + ((QK_weight, V_weight), )
            
        # return output, if only logits is needed, decompose it
        return output if len(output) > 1 else output[0]