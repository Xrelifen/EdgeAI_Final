...
class OffloadSDWrapper(SDWrapper):
    ...
    def set_llm(self, llm_path, memory_limit=6.0, device="cuda:0"):
        ...
        est_mem_usage += 512 * llm_config.vocab_size * 2 * 2 / (10 ** 9)
        head_dim = llm_config.hidden_size / llm_config.num_attention_heads
        kv_dim = head_dim * llm_config.num_key_value_heads
        est_mem_usage += 1024 * kv_dim * 2 * llm_config.num_hidden_layers * 2 * 2 / (10 ** 9)
        ...
        for block_n in range(llm_config.num_hidden_layers):
            for layer_n in range(len(llama_layer)):
                ...
                if est_mem_usage <= memory_limit - 3 * max_mem:
...
class OffloadShareSDWrapper(ShareSDWrapper):
    def __init__(self, draft_params: DraftParams, *model_args, **kwargs):
        super(OffloadShareSDWrapper, self).__init__(draft_params, *model_args, **kwargs)

    def set_llm(self, llm, memory_limit=8.0, device="cuda:0"):
        assert self.ssm is not None, "SSM model must first be loaded on gpu"
        # device_map = {
        #     "model.embed_tokens": "cuda:0",
        #     "model.norm": "cuda:0",
        ...
        # TODO: Make ssm & llm use same RMSNorm?
        # Set rotary_emb & rmsnorm to device
        for tensor_name, _ in named_module_tensors(self.llm.model.rotary_emb):
            set_module_tensor_to_device(self.llm.model.rotary_emb, tensor_name, device)
        for tensor_name, _ in named_module_tensors(self.llm.model.norm):
        ...
        # print("Check SSM ...")
        # for name, module in self.ssm.model.named_modules():
        #     device_set = set()
        #     for param in module.parameters(recurse=False):
        #         device_set.add(param.device)
        #     for buffer in module.buffers(recurse=False):
        #         device_set.add(buffer.device)
        #     print(f'Module {name}: {device_set}')

        # TODO: Make ssm & llm use same embed_tokens and lm_head, make sure on GPU
        ...
        # for tensor_name, _ in named_module_tensors(ssm_lm_head):
        #     set_module_tensor_to_device(ssm_lm_head, tensor_name, device)
        ...
        logging.info(f"[Mem Usage before dispatch] {est_mem_usage} GB")
        
        # Estimated Memory Usage for each sublayer
        llama_linear_layer = ['self_attn.q_proj', 'self_attn.k_proj', 'self_attn.v_proj', 'self_attn.o_proj', 'mlp.gate_proj', 'mlp.up_proj', 'mlp.down_proj']
        llama_nonlinear_layer = ['input_layernorm', 'post_attention_layernorm']
        
        max_mem = 0
        llama_layer_mem = {}
        for layer_name in llama_linear_layer + llama_nonlinear_layer:
            layer_mem = 0
            layer_name_split = layer_name.split('.')
        ...
        # Estimated kv_cache & activations
        est_mem_usage += 512 * llm_config.vocab_size * 2 * 2 / (10 ** 9)
        head_dim = llm_config.hidden_size / llm_config.num_attention_heads
        kv_dim = head_dim * llm_config.num_key_value_heads
        est_mem_usage += 1024 * kv_dim * 2 * llm_config.num_hidden_layers * 2 * 2 / (10 ** 9)

        # Estimate non-linear layers shared with ssm
        est_mem_usage += llm_config.num_hidden_layers * sum([mem for layer_name, mem in llama_layer_mem.items() if layer_name in llama_nonlinear_layer]) 

        # Set Device Map & Prefetch Map
        prefetch_name_map = {}
        module_map = {}
        device_map = {}
        ...
        # Non-Linear
        for block_n in range(llm_config.num_hidden_layers):
            for layer_n in range(len(llama_nonlinear_layer)):
                prefixed_layer_name = f'{block_n}.{llama_nonlinear_layer[layer_n]}'
                device_map[prefixed_layer_name] = device

        # Linear
        for block_n in range(llm_config.num_hidden_layers):
            for layer_n in range(len(llama_linear_layer)):
                layer_name = llama_linear_layer[layer_n]
                prefixed_layer_name = f'{block_n}.{layer_name}'
                
                next_layer_n = (layer_n+1) % len(llama_linear_layer)
                next_block_n = block_n + 1 if next_layer_n < layer_n else block_n
                prefixed_next_layer_name = f'{next_block_n}.{llama_linear_layer[next_layer_n]}'
                ...
                if est_mem_usage <= memory_limit - 3 * max_mem:
                    ...
                    device_map[prefixed_layer_name] = 'cpu'
                    if block_n != llm_config.num_hidden_layers - 1 or layer_n != len(llama_linear_layer) - 1:
                        prefetch_name_map[prefixed_layer_name] = prefixed_next_layer_name
