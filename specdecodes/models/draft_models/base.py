...
class TreeData(nn.Module):
    ...
    def get_data(self):
        ...
        self.parent_indices_data = torch.cat(self.parent_indices_data, dim=0).unsqueeze(
            0
        )
...
class TreeMaskCache:
    def __init__(
        ...
        # build dynamic tree_mask instead
            ...
            self.tree_mask_cache = torch.ones(
                (1, 1, 1, self.prefix_len), device=self.device, dtype=torch.bool
            )

        # Create an identity block for later use
        self.eye_block = (
            torch.eye(self.sample_len, device=self.device, dtype=torch.bool)
            .unsqueeze(0)
            .unsqueeze(0)
        )
    ...
    def update_tree_mask(self, parent_indices: torch.Tensor) -> torch.Tensor:
        if self.tree_mask_update_method == "static":  # static tree mask update
            ...
            self.tree_mask_cache[
                ..., self.current_len : self.current_len + self.sample_len
            ] = self.eye_block
...
class DraftModelBase(nn.Module):
    ...
    def from_pretrained(
        ...
        **model_kwargs,
        ...
        if os.path.exists(pretrained_model_name_or_path):
            logging.info(
                f"Loading local Eagle model from {pretrained_model_name_or_path}"
            )
            ...
            load_custom_model(
                model, draft_model_path, remove_embeddings=remove_embeddings
            )

        # HF Eagle safetensors
        elif use_hf_eagle:
            logging.info(
                f"Downloading Eagle safetensors from HF repo {pretrained_model_name_or_path}"
            )
            ...
            model = cls(
                base_model, eos_token_id=eos_token_id, *model_args, **model_kwargs
    ...
    @torch.no_grad()
    def topk_sampling(
        self, sampled_probs: torch.Tensor, parent_probs: torch.Tensor, sample_k: int
        ...
        with nvtx.annotate("sampling_2"):
            ...
            flattened_probs = global_probs.view(
                batch_size, -1
            )  # Shape: [N_available_leaves * vocab_size]
