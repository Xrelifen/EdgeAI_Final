...
class EagleSDDraftModel(DraftModelBase):
    ...
    def init_additional_modules(self):
        self.fusion = MergeLinear(
            self.config.hidden_size * 2, self.config.hidden_size, bias=self.bias
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        hidden_states = self.model(inputs_embeds=hidden_states, *model_args, **kwargs)[
            0
        ][:, -logits_to_keep:]
    ...
    def speculate(self, input_ids, hidden_states, past_key_values, **kwargs):
        ...
        with nvtx.annotate("init parent_probs & position_ids & cache_position"):
            ...
            cache_position = torch.arange(
                kv_len, org_input_len, dtype=torch.long, device=device
            )
        ...
        with nvtx.annotate("update cache"):
            cache_position = torch.arange(
                org_input_len,
                org_input_len + self.draft_params.topk_len,
                dtype=torch.long,
                device=device,
            )
        ...
        for depth_i in range(self.draft_params.max_depth):
            ...
            with nvtx.annotate("filter"):
                ...
                parent_indices_expanded = parent_indices.unsqueeze(-1).expand(
                    -1, -1, hidden_states.size(-1)
                )
                hidden_states = torch.gather(
                    hidden_states, dim=1, index=parent_indices_expanded
                )
