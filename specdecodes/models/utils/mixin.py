...
class ProfilingMixin:
    def __init__(self, *args, profiling: bool = True, **kwargs):
...
class SDProfilingMixin:
    def __init__(
        self, *args, profiling: bool = True, out_dir=None, prefix="sd", **kwargs
    ):
    ...
    def _verify(self, tree, *model_args, **kwargs):
        ...
        start_event.record()
        sampled_tokens, hidden_indices, (total_len, accept_len) = super()._verify(
            tree, *model_args, **kwargs
        )
        end_event.record()
    ...
    def compute_average_times(self):
        ...
        for start_event, end_event in self.draft_events:
            draft_time_total_ms += start_event.elapsed_time(
                end_event
            )  # returns time in ms
    ...
    def _generate(self, input_ids: torch.LongTensor, *model_args, **kwargs):
        ...
        # alpha (node)
        total_lens = torch.bincount(
            torch.tensor(self.profile_data["total_len"]), minlength=depth
        )
        accept_lens = torch.bincount(
            torch.tensor(self.profile_data["accept_len"]), minlength=depth
        )
        depth_total_cnt = (
            total_lens + total_lens.sum() - total_lens.cumsum(dim=-1)
        )  # reverse cumsum
        depth_total_cnt = depth_total_cnt[1:]  # remove first element
        depth_accept_cnt = (
            accept_lens + accept_lens.sum() - accept_lens.cumsum(dim=-1)
        )  # reverse cumsum
        ...
        sampled_lens = torch.tensor(
            [len(sampled_tokens) for sampled_tokens in self.profile_data["iter"]]
        )
        sampled_len_bins = torch.bincount(sampled_lens, minlength=depth + 1)
        depth_total_cnt = (
            sampled_len_bins + sampled_len_bins.sum() - sampled_len_bins.cumsum(dim=-1)
        )  # reverse cumsum
        ...
        tb.add_row(
            ["Alpha (depth)"] + [f"{val:.2f}" for val in alpha_per_depth.tolist()]
        )
        tb.add_row(
            ["Alive ratio"] + [f"{val:.2f}" for val in depth_alive_rate.tolist()]
        )
        logging.info(
