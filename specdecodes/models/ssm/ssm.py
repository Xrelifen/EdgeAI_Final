...
class SSM_Eagle(SSMBaseNEFT):
    ...
    def new_topk_sampling(
        ...
        sample_k: int, 
        sample_min_prob: float = 1e-8
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Optimized top-k sampling that remains on the GPU without synchronization.
        Samples `sample_k` new nodes in total across all available leaves.
        
        Args:
            sampled_probs (torch.Tensor): Tensor of shape [N_available_leaves, vocab_size] containing sampled probabilities.
            leaf_probs (torch.Tensor): Tensor of shape [N_available_leaves] containing probabilities for each leaf.
            sample_k (int): Total number of top tokens to sample across all leaves.
            sample_min_prob (float): Minimum probability threshold to flag a token.
        
        Returns:
            Tuple containing:
                - token_ids_array (torch.Tensor): Tensor of token IDs to add. Shape: [sample_k]
                - probabilities_array (torch.Tensor): Tensor of corresponding probabilities. Shape: [sample_k]
                - parent_indices_array (torch.Tensor): Tensor of parent leaf indices for each sampled token. Shape: [sample_k]
        ...
        # if found 29991, print
        # if torch.any(token_ids == 29991):
        #     print("Found 29991")
        #     print(token_ids)
        #     print(topk_probs)
    ...
    def speculate(self, input_ids, hidden_states, past_key_values, embed_tokens, lm_head, *model_args, **kwargs):
        ...
        # ! TODO: Use prev_sampling_index & cur_sampling_index to keep track of the in/out indices and logits.
        # ! Also consider pruning, think of a way to maintain all of this.
        # ! Compare tree indices before/after pruning
        
        #* 1. Take out all leaf nodes, drop the nodes with prob < min_accept_prob or eos_token_id
        #* 2. Keep track of the index of the new remaining leaf nodes
        
        #* 3. Prepare data and forward  (input_ids from leaf nodes, hidden_states from parent index, tree_mask and position_ids from leaf nodes)
        
        #* 4. 2.'s index becomes parent index
        #* 5. Sample the next nodes, append the topk nodes to their parent nodes
        #* 5. Repeat 1-4 until depth is reached
        #* 6. Crop the tree to the max_candidate_tokens
        
        # ! What we really need to keep track of, is the node indices and corresponding hidden_states index.
        # ! Hence, after pruning, we can obtain the indexes from mapping leaf nodes' parent indices.
        ...
        for i in range(self.depth):
            ...
            if old_depth != None and new_depth <= old_depth:
                break
            old_depth = new_depth
